...
AR1_dat = read_csv(paste(AR1_path, "adapted_brooksire_2024_TB2.csv", sep=""))
AR1_dat$p_raw <- format(AR1_dat$p_raw, scientific=F); AR1_dat$p_corr <- format(AR1_dat$p_corr, scientific=T)
# AR1_dat$dv = ifelse(AR1_dat$dv == ".D","IES",
#                     ifelse(AR1_dat$dv == "HR","HR",
#                            ifelse(AR1_dat$dv == "RT","RT",NA)))
AR1_dat
```
...
IDF_fac = getIDF_factors(inDat = RetDat_v4_res)
...
IDF_fac = getIDF_factors(inDat = EncDat_v4_res)
...
```{r}
pwr_model_pz = lm(prop_in_zone ~ nicotine + sex + cannabis + 
     anxiety + OCD + PTSD + MDD + scale(dayTime_shift), data = IDF_MemHr_dat_pz)
summary(pwr_model_pz)
confint(pwr_model_pz, method = "Wald")
```
...
```{r}
plot_path = paste(master_path,"Results/",sep="")
if (!dir.exists(paste(plot_path, "Figure_4", sep=""))) {
  dir.create(paste(plot_path, "Figure_4", sep=""))
}
# extras
...
p1 = ggplot(toPlot2, aes(allTrials, smoothed_deviance, col = zone)) + 
  ...
  ggtitle("Zone Metric Calculation") + theme(legend.position=c(.9,.9)) #+ coord_fixed(ratio = ar) 
p1
ggsave(paste(plot_path,"Figure_4/fig_4_a.pdf",sep=""), p1,width = 120, height = 40, units = "mm")
# --------------------------- Panel 2 --------------------------------
...
p2
ggsave(paste(plot_path,"Figure_4/fig_4_b.pdf",sep=""), p2,width = 50, height = 40, units = "mm")
# --------------------------- Panel 3 --------------------------------
p3 = mkCompSPD_2t_plot_v2(zp_mrt_sts,ylinRg = c(-3.5,5.7), ln_size = ln_size, fc=fc,
                              ...
                              yAxLab="RT Power Diff: In-Out", xAxLab="Frequency (Hz)") +
                                  annotate("segment", x = 4.5, xend = 6.5, y = 5.5, yend = 5.5, colour = "chartreuse4", size=ln_size)
p3
ggsave(paste(plot_path,"Figure_4/fig_4_c.pdf",sep=""), p3,width = 60, height = 40, units = "mm")
# --------------------------- Panel 4 --------------------------------
col1 = 'brown4'; col2 = 'sienna2'
# Nicotine effect
...
p4 = ggplot(IDF_MemHr_dat, aes(x=nicLab, y=resid)) + 
  geom_violin(aes(color=nicLab,fill=nicLab), alpha = .3, linewidth = ln_size, color=NA) + 
  ...
  labs(x = "Group")
p4
ggsave(paste(plot_path,"Figure_4/fig_4_d.pdf",sep=""), p4,width = 50, height = 40, units = "mm")
# --------------------------- Panel 5 --------------------------------
col1 = 'brown4'; col2 = 'sienna2'; 
# Nicotine effect
...
p5 = ggplot(IDF_MemHr_dat_pz, aes(x=nicLab, y=resid)) + 
  ...
  labs(x = "Group")
p5
ggsave(paste(plot_path,"Figure_4/fig_4_e.pdf",sep=""), p5,width = 50, height = 40, units = "mm")
